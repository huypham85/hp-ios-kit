...
description: |
  ...
  Explicit command: Users can also invoke this agent directly with `/audit storage`
# model: haiku
# color: blue
# tools:
  - Glob
...
- Use `/skill hp:storage` for storage decision framework
- Use `/skill hp:storage-diag` for debugging missing files
- Use `/skill hp:file-protection-ref` for encryption details
- Use `/skill hp:storage-management-ref` for purging policies
```
