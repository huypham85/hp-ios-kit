...
**Invoke**: Read the `ui-recording` skill
...
**Invoke**: Launch `simulator-tester` agent (uses axe-ref)
...
→ Invoke: simulator-tester (Agent) + axe-ref

User: "How do I tap a button using AXe?"
→ Invoke: axe-ref (via simulator-tester)
