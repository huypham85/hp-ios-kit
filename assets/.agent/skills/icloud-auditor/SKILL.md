...
description: |
  ...
  Explicit command: Users can also invoke this agent directly with `/audit icloud`
# model: haiku
# color: cyan
# tools:
  - Glob
...
- Use `/skill hp:cloud-sync-diag` for sync troubleshooting
- Use `/skill hp:cloudkit-ref` for modern CloudKit patterns
- Use `/skill hp:icloud-drive-ref` for file coordination details
```
