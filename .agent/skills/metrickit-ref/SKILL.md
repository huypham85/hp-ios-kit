...
For hang diagnosis workflows, see `hang-diagnostics`.
For general profiling with Instruments, see `performance-profiling`.
For memory debugging including jetsam, see `memory-debugging`.
...
**Skills**: hang-diagnostics, performance-profiling, testflight-triage
