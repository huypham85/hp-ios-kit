...
description: Use when first installing HP, unsure which skill to use, want an overview of available skills, or need help finding the right skill for your situation — interactive onboarding that recommends skills based on your project and current focus
...
Welcome! This skill helps new users discover the most relevant HP skills for their situation.
...
```
Question: "What brings you to HP today?"
Header: "Focus"
Options:
...
**Always recommend**: hp:xcode-debugging (universal starting point)
...
1. `/audit-accessibility` — WCAG compliance
2. `/audit-concurrency` — Swift 6 violations
3. `/audit-memory` — Leak patterns
4. `/audit-core-data` — Migration safety
5. `/audit-networking` — Deprecated APIs
...
### [Icon] [Category Name]
**hp:[skill-name]** — [One-line description]
...
- `/audit-accessibility` — VoiceOver labels, Dynamic Type, contrast, touch targets
- `/audit-concurrency` — Swift 6 violations, unsafe tasks, missing @MainActor
- `/audit-memory` — Timer leaks, observer leaks, closure captures, delegate cycles
- `/audit-core-data` — Migration risks, thread violations, N+1 queries
- `/audit-networking` — Deprecated APIs (SCNetworkReachability, CFSocket), anti-patterns
- `/audit-liquid-glass` — Glass adoption opportunities, toolbar improvements, blur migration
...
**"My build is failing"**
→ Start: hp:xcode-debugging
→ If dependency issue: hp:build-debugging

**"App is slow"**
→ Start: hp:performance-profiling (decision trees)
→ If SwiftUI: hp:swiftui-performance
→ If memory grows: hp:memory-debugging

**"Memory leak"**
→ Start: hp:memory-debugging
→ If Objective-C blocks: hp:objc-block-retain-cycles

**"SwiftUI view issues"**
→ Start: hp:swiftui-debugging
→ If performance: hp:swiftui-performance

**"Navigation problems"**
→ Start: hp:swiftui-nav-diag (troubleshooting)
→ For patterns: hp:swiftui-nav

**"Which database?"**
→ Decision guide: hp:sqlitedata-migration
→ Then: hp:swiftdata, hp:sqlitedata, or hp:grdb

**"iOS 26 design"**
→ Start: hp:liquid-glass
→ Complete guide: hp:liquid-glass-ref

**"Code quality check"**
→ Run: `/audit-accessibility`, `/audit-concurrency`, `/audit-memory`
