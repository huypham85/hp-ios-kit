...
description: |
  ...
  Explicit command: Users can also invoke this agent directly with `/audit swiftui-architecture`
# model: haiku
# color: blue
# tools:
  - Glob
...
- `OrderList.swift:88` - Filtering and sorting in `body`
  ...
  - **Note**: This also impacts performance (see `/audit swiftui-performance`)

## Recommendations
1. Fix CRITICAL issues first (bugs)
2. Extract logic from views to models (testability)
3. If performance is a concern, run `/audit swiftui-performance`
```
